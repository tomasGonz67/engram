...
3. **A renewal hook restarts the backend after a successful certbot renewal** — Uvicorn doesn't hot-reload cert files, so a renewed cert only takes effect after a restart. A few seconds of dropped connections each renewal (roughly every 60 days, well inside the 90-day validity) is an accepted tradeoff for this project's traffic pattern, not something worth adding zero-downtime cert reloading to solve. **Done**: `/etc/letsencrypt/renewal-hooks/deploy/restart-backend.sh` (`cd /opt/masi-memory && docker compose restart backend`), picked up automatically by certbot's existing renewal mechanism since anything executable in that directory runs after a successful renewal — no changes to the renewal config itself needed. Verified two ways: a `certbot renew --dry-run` confirmed the hook is correctly registered (certbot logs "Dry run: skipping deploy hook command" — dry-runs deliberately skip deploy-hooks by design, so this only proves registration, not execution); running the script directly confirmed the actual behavior — the backend container restarted cleanly and came back up healthy (`https://134-209-121-221.sslip.io/docs` returned `200` again within seconds).
