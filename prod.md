...
- **Domain/DNS**: no purchased domain. Frontend uses Cloudflare Pages' free `*.pages.dev` subdomain. Backend uses a free `sslip.io` hostname (e.g. `<droplet-ip-with-dashes>.sslip.io`) instead of a bare IP — Let's Encrypt won't issue a trusted certificate for a bare IP address, and sslip.io's auto-resolving wildcard DNS gets a real, trusted cert via Traefik's existing HTTP-01 challenge without paying for a domain.
