...
**This section describes the investigation that led to the reranking-weight decision — it is not a description of current production behavior.** Current production weights are `SEMANTIC_WEIGHT=0.95`/`RETRIEVABILITY_WEIGHT=0.00`/`FREQUENCY_WEIGHT=0.05` (`constants.py`), adopted specifically because they fixed everything this section describes. See "Current status" below for where things actually stand now; the rest of this section is preserved as the evidence trail, not current fact.
