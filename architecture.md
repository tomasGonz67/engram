...
This is a weighted **sum**, not a product — deliberately, so that a weak signal on one term (e.g. a brand-new memory with `use_count = 0`) can't zero out the whole score the way a product would. Semantic similarity dominates (75%) so an irrelevant memory can't win purely by being old or heavily used; the semantic threshold is an additional hard floor on top of that weighting, applied before re-ranking. `SEMANTIC_THRESHOLD` (`constants.py`) is currently **0.8** — still a placeholder, not a fully validated value, but meaningfully better calibrated than the original guess of `0.7`. A genuine nonsense query ("asdkjf qwoeiru zzxcvbn blorp glorp nonsense gibberish") tested against a 60-memory seeded dataset scored `0.716`–`0.768`, all of which wrongly passed the old `0.7` threshold; known-relevant matches have all scored `0.834`+. `0.8` sits between those two ranges. Still worth real calibration once there's larger, more systematic usage data — see `constants.py`.
