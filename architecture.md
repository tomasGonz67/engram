...
Both completion calls go through a shared helper that catches OpenAI API failures (rate limit, timeout, outage) and returns a `502` instead of letting an unhandled exception become an opaque `500` — see `security-preventions.md`'s Resolved section.
